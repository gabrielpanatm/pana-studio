...
const localeModules = locales.map((locale) => ({
  locale,
...
const localeManifests = Object.fromEntries(
  locales.map((locale) => [locale, catalog[locale].manifest]),
);
const messageIdType = messageIds.length > 0
  ? messageIds.map((id) => `  | ${JSON.stringify(id)}`).join("\n")
  : "never";
const generated = `/* This file is generated by scripts/generate-i18n-catalog.mjs. */\n`
  ...
  + `export const localeManifests = ${JSON.stringify(localeManifests, null, 2)} as const;\n`
  ...
  + localeModules
    .map(({ locale }) =>
      `  ${JSON.stringify(locale)}: () => import(${JSON.stringify(`./catalog.${locale}`)}),\n`
    )
    .join("")
  + `} as const;\n`
