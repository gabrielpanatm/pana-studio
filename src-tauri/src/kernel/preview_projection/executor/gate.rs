...
pub(super) fn require_preview_executor_intent(
    ...
    let diagnostic = if wrong_kind {
        Some(PreviewProjectionDiagnostic::blocking(
            ...
            LocalizedDiagnostic::new("preview-projection-wrong-executor-intent")
                .with_argument("executor", spec.wrong_kind_code),
        ))
    } else {
        None
    };
...
mod tests {
    ...
    use super::super::spec::{HTML_DELETE_INTENT, HTML_INSERT_DROP_INTENT};
    use super::*;
    ...
    fn require_preview_executor_intent_adds_wrong_kind_diagnostic() {
        ...
        let blocked = require_preview_executor_intent(input, &session(), HTML_INSERT_DROP_INTENT)
        ...
        assert_eq!(
            ...
            "preview_html_insert_drop_wrong_intent_kind"
        );
    }
}
