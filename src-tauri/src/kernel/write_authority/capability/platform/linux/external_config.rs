use super::{
    capability_error, capture_existing_target_parent, capture_parent_from_wal_evidence,
    encode_component_hex, external_config_backup_temp_leaf, external_config_target_temp_leaf, fs,
    lexical_target, run_test_hook, same_stable_leaf_version, sha256_bytes, sync_directory,
    validate_named_file_identity, validate_regular_single_link, wal_evidence_from_open_file,
    wal_identity_from_fd, wal_recovery_effect, CapabilityEffect, CapabilityTestStage,
    DurableWalGuard, ExpectedLeaf, ExternalConfigOperationPlan, File, Mode, OFlags, OsStr, OwnedFd,
    RenameFlags, WalAtomicFileEvidence, WalExternalConfigEvidence, WalExternalStageCheckpoint,
    WalLeafEvidence, WalParentEvidence, WriteTarget, MAX_WAL_EXTERNAL_CONFIG_BYTES,
    WAL_EXTERNAL_CONFIG_PROTOCOL_VERSION,
};
#[cfg(test)]
use std::path::Path;
...
#[cfg(test)]
use snapshot::external_stage_identity_digest;
use snapshot::{
    external_auxiliary_leaves, external_baseline_identity_digest, external_mode_bits,
    leaf_matches_payload, owned_external_leaves, publish_external_anonymous,
    recapture_external_public_parent, stage_external_anonymous,
    validate_external_leaf_distinctness, validate_external_payload_size,
    validate_external_plan_shape, validate_leaf_absent_for_external,
    validate_open_external_baseline, validate_open_new_payload, validate_runtime_before,
    ExternalLeaves,
};
