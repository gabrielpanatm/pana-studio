use super::{
    capability_error, capture_directory_authority, capture_parent_from_wal_evidence,
    decode_component_hex, decode_path_hex, encode_bytes_hex, encode_path_hex, fs, leaf_metadata,
    lexical_target, open_directory_strict, run_test_hook, same_stable_leaf_version, sync_directory,
    validate_named_directory_identity, version_token_for_stat, wal_identity_from_fd,
    wal_recovery_effect, AtFlags, CapabilityEffect, CapabilityTestStage, DirectoryAuthority,
    DirectoryAuthorityScope, DurableWalGuard, Errno, FileType, LexicalTarget, OsStr, OsString,
    OwnedFd, Path, SymlinkOperationPlan, SymlinkRecoveryAction, SymlinkRecoveryAssessment,
    WalFilesystemIdentity, WalOperationEvidence, WalPhase, WalRecord, WalSymlinkBefore,
    WalSymlinkEvidence, WriteAuthorityRecoveryClassification, WriteTarget,
    MAX_WAL_SYMLINK_TARGET_BYTES,
};
#[cfg(test)]
use super::{capture_existing_boundary, encode_component_hex, ExpectedLeaf, WalParentEvidence};
...
#[cfg(test)]
pub(super) fn plan_legacy_symlink(
    target: &WriteTarget,
    source: &Path,
...
pub(super) fn symlink_entry_legacy_wal(
    target: &WriteTarget,
    source: &Path,
...
pub(super) fn classify_legacy_symlink_recovery(
    record: &WalRecord,
    phase: WalPhase,
...
pub(super) fn execute_legacy_symlink_recovery(
    record: &WalRecord,
    phase: WalPhase,
