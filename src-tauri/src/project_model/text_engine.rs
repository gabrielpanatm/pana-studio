...
pub fn plan_html_text(model: &ProjectModel, intent: &ProjectHtmlTextIntent) -> ProjectHtmlTextPlan {
    match plan_html_text_inner(model, intent) {
...
mod tests {
    ...
    #[test]
    fn plan_html_text_rejects_location_without_source_id() {
        let root = unique_test_dir();
        let mut fixture =
