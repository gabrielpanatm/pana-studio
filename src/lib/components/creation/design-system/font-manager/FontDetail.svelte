...
{#if font}
  ...
  <section class="font-delivery-actions" aria-labelledby="font-delivery-title"><div><span id="font-delivery-title">{t("design-browser-delivery")}</span><small>{t("design-browser-delivery-description")}</small></div><div class="ui-form-field font-display-field"><span class="ui-form-label">{t("design-font-display-policy")}</span><SelectControl value={font.registration.displayModes.length === 1 ? font.registration.displayModes[0] : ""} options={[...(font.registration.displayModes.length !== 1 ? [{ value: "", label: t("design-choose-policy") }] : []), { value: "swap", label: t("design-display-swap") }, { value: "optional", label: t("design-display-optional") }, { value: "fallback", label: t("design-display-fallback") }, { value: "block", label: t("design-display-block") }, { value: "auto", label: t("design-display-auto") }]} disabled={controller.mutating || !font.registration.managed} ariaLabel={t("design-font-display-policy")} onchange={(value) => { const display = value as "auto" | "block" | "swap" | "fallback" | "optional"; if (display) void controller.changeSelectedFontDisplay(display); }} /></div>{#if !font.registration.managed}<small>{t("design-display-managed-only")}</small>{/if}</section>
  ...
  {#if controller.formError}<p class="ui-message error font-action-error" role="alert"><IconAlertTriangle size={14} /> {controller.formError}</p>{/if}
...
<style>
  ...
  .font-display-field { grid-template-columns: minmax(100px, 1fr) minmax(150px, 1.2fr); align-items: center; }
  ...
  .font-removal > :global(.ui-button) { justify-self: start; }
</style>
