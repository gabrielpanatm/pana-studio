...
<form class="resource-form" onsubmit={(event) => { event.preventDefault(); void controller.installSelectedFont(); }}>
  ...
  {#if controller.fontCreateSource === "google"}
    ...
    {#if controller.selectedGoogleFont}
      <div class="font-install-options">
        ...
        {#if controller.selectedGoogleFont.axes.some((axis) => axis.tag === "wght")}<label class="check-field"><input checked={controller.formVariableFont} type="checkbox" disabled={controller.mutating} onchange={(event) => controller.setVariableFont(event.currentTarget.checked)} /><span>{t("design-full-variable-range")}</span></label>{/if}
        ...
        {/if}
        <label class="font-character-set"><span>{t("design-character-optimization")}</span><textarea bind:value={controller.formGoogleCharacterSet} disabled={controller.mutating} maxlength="640" rows="3" placeholder={t("design-character-example")}></textarea><small>{t("design-character-help")}</small></label>
      </div>
    {/if}
  {:else if controller.fontCreateSource === "bundled"}
    ...
    <div class="bundled-font-filters"><input bind:value={controller.bundledFontQuery} disabled={controller.mutating || controller.bundledFontLoading} aria-label={t("design-search-family")} placeholder={t("design-search-family")} /><select bind:value={controller.bundledFontCategory} disabled={controller.mutating || controller.bundledFontLoading} aria-label={t("design-font-category")}><option value="all">{t("design-all-categories")}</option>{#each controller.bundledFontCategories as category (category)}<option value={category}>{category}</option>{/each}</select></div>
    {#if controller.bundledFontError}<p class="form-error" role="alert"><IconAlertTriangle size={14} /> {controller.bundledFontError}</p>
...
<style>
  ...
  .google-search input, .bundled-font-filters input, .bundled-font-filters select { min-width: 0; height: 34px; padding: 0 9px; border: 1px solid var(--wb-border-subtle); border-radius: 6px; color: var(--text-strong); background: var(--wb-surface-document); font-size: 12px; }
  .google-search input { border-right: 0; border-radius: 6px 0 0 6px; }
  .google-search button { display: inline-flex; min-width: 82px; align-items: center; justify-content: center; gap: 5px; padding: 0 9px; border: 1px solid var(--wb-border-subtle); border-radius: 0 6px 6px 0; color: var(--wb-text-primary); background: var(--wb-control-hover); font-size: 12px; font-weight: 700; }
  ...
  .font-character-set { display: grid; gap: 5px; color: var(--wb-text-muted); font-size: 11px; font-weight: 700; }
  .font-character-set textarea { min-height: 56px; padding: 7px 8px; resize: vertical; border: 1px solid var(--wb-border-subtle); border-radius: 5px; color: var(--wb-text-primary); background: var(--wb-surface-chrome); }
  ...
  .plan-message { display: flex; gap: 6px; margin: 0; padding: 8px; border: 1px solid var(--wb-border-subtle); border-radius: 6px; color: var(--warning); font-size: 11px; }
</style>
