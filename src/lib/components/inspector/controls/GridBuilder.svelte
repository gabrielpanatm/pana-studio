...
<div class="grid-builder" data-grid-builder>
  <div class="grid-builder-heading">
    ...
    <button
      type="button"
      class="overlay-toggle ui-icon-button mini"
  ...
  <div class="axis-block ui-card">
    <div class="axis-heading">
      <strong>{t("inspector-grid-columns")}</strong>
      <button type="button" class="ui-icon-button mini" disabled={columnsBlocked} title={t("inspector-grid-add-column")} aria-label={t("inspector-grid-add-column")} onclick={() => addTrack("columns")}>
        ...
        {#each grid.templateColumns.tracks as track, index (track.id)}
          <div class="track-card ui-card">
            ...
            <div class="track-actions">
              <button type="button" class="ui-icon-button mini" disabled={columnsBlocked || index === 0} title={t("inspector-grid-move-before")} aria-label={t("inspector-grid-move-before")} onclick={() => moveTrack("columns", track.id, -1)}><IconArrowUp size={12} /></button>
              <button type="button" class="ui-icon-button mini" disabled={columnsBlocked || index === grid.templateColumns.tracks.length - 1} title={t("inspector-grid-move-after")} aria-label={t("inspector-grid-move-after")} onclick={() => moveTrack("columns", track.id, 1)}><IconArrowDown size={12} /></button>
              <button type="button" class="ui-icon-button mini" disabled={columnsBlocked} title={t("inspector-grid-duplicate-track")} aria-label={t("inspector-grid-duplicate-track")} onclick={() => duplicateTrack("columns", track.id)}><IconCopy size={12} /></button>
              <button type="button" class="ui-icon-button mini danger" disabled={columnsBlocked} title={t("inspector-grid-remove-track")} aria-label={t("inspector-grid-remove-track")} onclick={() => removeTrack("columns", track.id)}><IconTrash size={12} /></button>
            </div>
    ...
    {:else}
      <div class="empty-axis">
        ...
        <button type="button" class="ui-button compact quiet" disabled={columnsBlocked} onclick={() => addTrack("columns")}>{t("inspector-grid-add-column")}</button>
      </div>
    {/if}
  </div>
  ...
  <div class="axis-block ui-card">
    <div class="axis-heading">
      ...
      <button type="button" class="ui-icon-button mini" disabled={rowsBlocked} title={t("inspector-grid-add-row")} aria-label={t("inspector-grid-add-row")} onclick={() => addTrack("rows", "keyword")}>
        <IconPlus size={13} stroke={2} />
      </button>
    </div>
      ...
      <div class="track-list">
        {#each grid.templateRows.tracks as track, index (track.id)}
          <div class="track-card ui-card">
            ...
            <div class="track-actions">
              <button type="button" class="ui-icon-button mini" disabled={rowsBlocked || index === 0} title={t("inspector-grid-move-before")} aria-label={t("inspector-grid-move-before")} onclick={() => moveTrack("rows", track.id, -1)}><IconArrowUp size={12} /></button>
              <button type="button" class="ui-icon-button mini" disabled={rowsBlocked || index === grid.templateRows.tracks.length - 1} title={t("inspector-grid-move-after")} aria-label={t("inspector-grid-move-after")} onclick={() => moveTrack("rows", track.id, 1)}><IconArrowDown size={12} /></button>
              <button type="button" class="ui-icon-button mini" disabled={rowsBlocked} title={t("inspector-grid-duplicate-track")} aria-label={t("inspector-grid-duplicate-track")} onclick={() => duplicateTrack("rows", track.id)}><IconCopy size={12} /></button>
              <button type="button" class="ui-icon-button mini danger" disabled={rowsBlocked} title={t("inspector-grid-remove-track")} aria-label={t("inspector-grid-remove-track")} onclick={() => removeTrack("rows", track.id)}><IconTrash size={12} /></button>
            </div>
            {#if track.kind === "repeat"}
              <div class="repeat-fields">
    ...
    {:else}
      <div class="empty-axis">
        ...
        <button type="button" class="ui-button compact quiet" disabled={rowsBlocked} onclick={() => addTrack("rows", "keyword")}>{t("inspector-grid-add-row")}</button>
      </div>
    {/if}
  </div>
  ...
  <div class="field-heading">
    ...
    <button type="button" class="ui-icon-button mini" aria-pressed={gapsLinked} title={gapsLinked ? t("inspector-grid-unlink-gaps") : t("inspector-grid-link-gaps")} onclick={() => { gapsLinked = !gapsLinked; }}>
  ...
  <button type="button" class="advanced-toggle ui-button compact quiet" aria-expanded={advancedOpen} onclick={() => { advancedOpen = !advancedOpen; }}>
    {t("inspector-grid-advanced")}
  </button>
  {#if advancedOpen}
    <div class="advanced-fields ui-card">
...
<style>
  ...
  .overlay-toggle { flex: 0 0 auto; color: var(--brand-strong); }
  ...
  .axis-block, .areas-block, .advanced-fields { display: grid; gap: 6px; padding: 7px; }
  .track-list { display: grid; gap: 5px; }
  .track-card { display: grid; grid-template-columns: 24px minmax(76px, .72fr) minmax(0, 1fr); align-items: center; gap: 5px; padding: 5px; }
  .track-index { display: grid; height: 24px; place-items: center; border-radius: 4px; color: var(--brand-strong); background: var(--brand-soft); font: 700 11px/1 var(--font-mono); }
  ...
  .areas-block textarea { min-height: 64px; resize: vertical; font-size: 11px; line-height: 1.5; }
