...
export function buildHtmlSnippet(options: HtmlSnippetOptions) {
  ...
  if (tag === "hgroup") return `<hgroup${attrs}><h2>${textOr(text, "Heading group")}</h2><p>Subtitle</p></hgroup>`;
  if (tag === "details") return `<details${attrs}><summary>${textOr(text, "Details")}</summary><p>Details content</p></details>`;
  if (tag === "dialog") return `<dialog${attrs} open>${textOr(text, "New dialog")}</dialog>`;
  if (tag === "bdo") return `<bdo${attrs} dir="ltr">${textOr(text, "Text")}</bdo>`;
  if (tag === "data") return `<data${attrs} value="">${textOr(text, "Value")}</data>`;
  if (tag === "ruby") return `<ruby${attrs}>${textOr(text, "Text")}<rp> (</rp><rt>Pronunciation</rt><rp>)</rp></ruby>`;
  ...
  if (tag === "track") return `<track${attrs} src="" kind="captions" srclang="ro" label="Română">`;
  ...
  if (tag === "canvas") return `<canvas${attrs} width="300" height="150">${textOr(text, "Canvas unavailable")}</canvas>`;
  if (tag === "object") return `<object${attrs} data="" type=""><p>${textOr(text, "Content unavailable")}</p></object>`;
  if (tag === "embed") return `<embed${attrs} src="" type="">`;
  if (tag === "map") return `<map${attrs} name="harta"><area shape="rect" coords="" href="#" alt="Zonă"></map>`;
  if (tag === "area") return `<area${attrs} shape="rect" coords="" href="#" alt="${escapeAttr(text || "Area")}">`;
  if (tag === "picture") return `<picture${attrs}><img src="" alt="${escapeAttr(text || "Image")}"></picture>`;
  if (tag === "template") return `<template${attrs}><div>${textOr(text, "Template content")}</div></template>`;
  if (tag === "slot") return `<slot${attrs}>${textOr(text, "Fallback content")}</slot>`;
  if (tag === "ul") return `<ul${attrs}><li>${textOr(text, "List item")}</li></ul>`;
  if (tag === "ol") return `<ol${attrs}><li>${textOr(text, "List item")}</li></ol>`;
  if (tag === "menu") return `<menu${attrs}><li>${textOr(text, "Menu item")}</li></menu>`;
  ...
  if (tag === "optgroup") return `<optgroup${attrs} label="${escapeAttr(text || "Group")}"><option>Option</option></optgroup>`;
  if (tag === "datalist") return `<datalist${attrs}><option value="${escapeAttr(text || "Option")}"></option></datalist>`;
  if (tag === "fieldset") return `<fieldset${attrs}><legend>${textOr(text, "Legend")}</legend></fieldset>`;
  if (tag === "progress") return `<progress${attrs} value="0" max="100">0%</progress>`;
  if (tag === "meter") return `<meter${attrs} min="0" max="100" value="0">0</meter>`;
  if (tag === "table") return `<table${attrs}><tbody><tr><td>${textOr(text, "Cell")}</td></tr></tbody></table>`;
  if (tag === "colgroup") return `<colgroup${attrs}><col></colgroup>`;
