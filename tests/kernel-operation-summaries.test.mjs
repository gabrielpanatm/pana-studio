...
test("all editor mutation command families stage through ProjectWorkspace", () => {
  ...
  for (const path of commandFiles) {
    ...
    assert.match(
      rust,
      /commit_project_workspace_session_mutation|publish_prepared_project_workspace_candidate/,
      path,
    );
...
test("HTML structural mutations publish one workspace revision and never write project files directly", () => {
  ...
  assert.match(pipeline, /capture_preview_write_workspace_candidate/);
  assert.match(pipeline, /with_preview_write_workspace_cas/);
  assert.match(pipeline, /publish_prepared_project_workspace_candidate/);
...
test("legacy parallel-authority modules and symbols are absent", () => {
  const removedFiles = [
    ...
    "src/lib/html/generated-identity.ts",
  ...
  const forbidden = [
    ...
    "EditorLayerContextMenuRequest",
    "offset_for_source_location",
  ];
